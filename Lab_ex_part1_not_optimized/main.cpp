...
const double C[I*J] = { 1000.0,   6.0,  4.0,   7.2, 11.2, 11.0, 11.2, 11.7, 12.5, 14.2,  
						6.0,   1000.0,  7.2,   4.0, 13.6, 13.0, 11.7, 11.2, 11.0, 11.2,
						4.0,   7.2,  1000.0,   6.0,  7.3,  7.0,  7.3,  8.1,  7.3,  9.2,
						7.2,   4.0,  6.0,   1000.0, 10.6,  9.9,  8.6,  7.6,  7.1,  7.3,
						11.2, 13.6,  7.3,  10.6,  1000.0,  2.0,  4.0,  6.0,  8.0, 11.0,
						11.0, 13.0,  7.0,   9.9,  2.0,  1000.0,  2.0,  4.0,  6.0,  9.0,
						11.2, 11.7,  7.3,   8.6,  4.0,  2.0,  1000.0,  2.0,  4.0,  7.0,
						11.7, 11.2,  8.1,   7.6,  6.0,  4.0,  2.0,  1000.0,  2.0,  5.0,
						12.5, 11.0,  7.3,   7.1,  8.0,  6.0,  4.0,  2.0,  1000.0,  3.0,
						14.2, 11.2,  9.2,   7.3, 11.0,  9.0,  7.0,  5.0,  3.0,  1000.0 }; 
						// costs(origin, destination) LINEARIZED (just an implementation choice!)
...
void setupLP(CEnv env, Prob lp)
{
	...
	for (int j = 0; j < J; j++)
	{
		...
		for (int i = 0; i < I; i++)
		{
			idx[i] = I*J + i*J + j; // corresponds to variable y_ij
		}
		int matbeg = 0;
		double rhs = 1;
	...
	for (int i = 0; i < I; i++)
	{
		...
		for (int j = 0; j < J; j++)
		{
			idx[j] = I*J + i*J + j; // corresponds to variable y_ij
		}
		int matbeg = 0;
		double rhs = 1;
		CHECKED_CPX_CALL( CPXaddrows, env, lp, 0     , 1     , idx.size(), &rhs, &sense, &matbeg, &idx[0], &coef[0], NULL      , NULL      );
	...
	for (int k = 1; k < I; k++)
	{
		...
		for (int i = 0; i < I; i++)
		{
			idx[i] = i*J + k; // corresponds to variable x_ik
		}
		for (int j = 0; j < J; j++)
		{
			idx[I+j] = k*J + j; // corresponds to variable x_kj
			coef[I+j] = -1; 
		}
		int matbeg = 0;
		double rhs = 1;
...
int main (int argc, char const *argv[])
{
	try
	{
		...
		if (n != 2*I*J) { throw std::runtime_error(std::string(__FILE__) + ":" + STRINGIZE(__LINE__) + ": " + "different number of variables"); }
